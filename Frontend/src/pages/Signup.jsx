...
function Signup() {
    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            console.log("Error from handle signup", error)
            setUserData(null)
    ...
    return (
        < div className='w-full h-[100vh] bg-no-repeat bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }
            ...
            <form className='w-[90%] h-[550px] max-w-[500px] bg-[#00000041] backdrop-blur shadow-black-950 flex flex-col items-center justify-center gap-[20px] px-[20px]' onSubmit={handleSignup}>
                <h1 className='text-white text-[30px] font-semibold mb-[30px]' >
                    Register to <span className='text-blue-400'>Virtual Assistance</span></h1>

                <input type="text" placeholder='Enter your name' name='name' className='w-full h-[60px] outline-none border-2 border-white text-white placeholder-gray-300 bg-transparent px-[20px] py-[10px] rounded-full text-[18px]' onChange={(e) => setName(e.target.value)} value={name} />
                ...
                {err && err.length > 0 && <p className='text-red-500 text-[17px]'>
                    *{err}</p>}
                ...
                <p className='text-white text-[18px] cursor-pointer'>Already have an account ? <span className='text-blue-400' onClick={() => navigate('/signin')}>SignIn</span></p>
